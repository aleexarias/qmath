...
def synthetic_heston_chain(
    ...
    r"""
    Generate a synthetic option chain from a Black-Scholes model with smile.
    ...
    Notes
    -----
    For now, uses Black-Scholes with a volatility smile (moneyness-dependent
    vol).
    ...
    for i in range(1, n_strikes - 1):
        c_up = bs_price(
            spot_arr,
            np.asarray(strikes[i] + dk, dtype=np.float64),
            T,
            rate,
            smile_vol[i],
            flag="C",
        ...
        true_density[i] = (
            (float(c_up) - 2 * call_prices[i] + float(c_down)) / (dk**2) / df
        )
    ...
    bid_calls = (
        mid_calls - spread_abs / 2 + rng.normal(0, spread_abs / 4, n_strikes)
    )
    ask_calls = (
        mid_calls + spread_abs / 2 + rng.normal(0, spread_abs / 4, n_strikes)
    )
